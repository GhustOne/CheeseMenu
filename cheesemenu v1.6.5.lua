...
if stuff.input then
	function input.get(title, default, len, Type)
		...
		--[[while func.get_key(0x08):is_down() do
			system.wait(0)
		end--]]
	...
	__index = function(t, k)
		...
		elseif stuff.playerSpecialValues[k] then
			if t["table_"..k:gsub("real_", "")] then
				return t["table_"..k:gsub("real_", "")][stuff.pid]
			end
		else
			return stuff.rawget(t, k)
		end
...
function func.add_feature(nameOfFeat, TypeOfFeat, parentOfFeat, functionToDo, playerFeat, playerid)
	...
	currentParent[#currentParent + 1] = {name = nameOfFeat, real_type = stuff.type_id.name_to_id[TypeOfFeat], real_id = tostring(parentOfFeat).."-"..tostring(#currentParent + 1), parent = {id = currentParent.id or 0}, parent_id = currentParent.id or 0, playerFeat = playerFeat, index = #currentParent + 1}
...
function func.add_player_feature(nameOfFeat, TypeOfFeat, parentOfFeat, functionToDo)
	...
	if next(featIds) then
		for k, v in pairs(featIds) do
			...
			currentParent[pfeatID].playerid = k-1
		...
		for k, v in pairs(stuff.playerIds) do
			...
			stuff.playerIds[k][pfeatID].playerid = k
...
function func.set_player_feat_parent(nameOfFeat, parentOfFeat, functionToDo)
	...
	event.add_event_listener("player_join", function(listener)
		...
		--if not (v.name == player.get_player_name(k)) then
			...
			func.add_to_table(features["OnlinePlayers"], stuff.playerIds[listener.player], listener.player, true)
		--end
	end)
...
backgroundfeat:set_str_data({"NONE", table.unpack(stuff.menuData.files.background)})
...
backgroundoffsety.value = math.floor(stuff.menuData.background_sprite.offset.y*graphics.get_screen_height())
...
menu.add_feature("Draw footer", "toggle", menu_configuration_features.footer.id, function(f)
	stuff.menuData.footer.draw_footer = f.on
end).on = stuff.menuData.footer.draw_footer

menu.add_feature("Footer position based on background", "toggle", menu_configuration_features.footer.id, function(f)
	stuff.menuData.footer.footer_pos_related_to_background = f.on
end).on = stuff.menuData.footer.footer_pos_related_to_background
